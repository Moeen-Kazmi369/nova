...
export const VoiceControls: React.FC<VoiceControlsProps> = ({
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      setIsListening(true);
      onSpeechStart();
      if (permissionStatusCallback) permissionStatusCallback('granted');
    } catch (err) {
      setError('Microphone access denied or not available.');
      if (permissionStatusCallback) permissionStatusCallback('denied');
    }
  };
  ...
  return (
    <div className="flex flex-col items-center">
      <button
        ...
        title={isListening ? 'Stop recording' : isLoading ? 'Transcribing...' : 'Start voice input'}
      >
        {isLoading ? (
      ...
      {isListening && <div className="text-xs text-red-400 mt-1">Recording...</div>}
